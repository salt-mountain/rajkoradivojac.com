...
function parseEvent(ev: SesEvent): {
  ...
  if (type === "Reject") {
    return {
      type,
      emails: ev.mail?.destination ?? [],
      reason: ev.reject?.reason ?? null,
      suppress: false,
    };
  }
  return {
