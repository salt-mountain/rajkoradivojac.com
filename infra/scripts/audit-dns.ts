...
async function listRecords(
  ...
  const res = await fetch(
    `${CF_API}/zones/${zoneId}/dns_records?per_page=200`,
    {
...
async function main(): Promise<void> {
  ...
  if (!zoneId)
    throw new Error("Set CLOUDFLARE_ZONE_ID (zone id for rajkoradivojac.com).");
  ...
  const dmarc = records.filter(
    (r) => r.type === "TXT" && r.name === `_dmarc.${APEX}`,
  );
  ...
  console.log(
    `Apex MX (Cloudflare Email Routing — MUST PRESERVE): ${apexMx.length || "none"}`,
  );
  ...
  console.log(
    `Existing _dmarc (apply-dns won't overwrite without --overwrite-dmarc): ${dmarc.length || "none"}`,
  );
  dmarc.forEach((r) => console.log(fmt(r)));
  console.log(`Existing DKIM (_domainkey): ${dkim.length || "none"}`);
  dkim.forEach((r) => console.log(fmt(r)));
  console.log(
    `Existing mail.${APEX} records (SES MAIL FROM target): ${mailFrom.length || "none"}`,
  );
