...
export class EmailStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: EmailStackProps) {
    ...
    new ses.ConfigurationSetEventDestination(
      this,
    ...
    sesAlarm("ComplaintRateAlarm", {
      ...
      description:
        "SES complaint rate above 0.1% — risk of SES sending suspension.",
    ...
    sesAlarm("SendVolumeAlarm", {
      ...
      description:
        "SES sent more than 200 emails in a day — possible sender-key misuse.",
      metricName: "Send",
      statistic: "Sum",
    ...
    out(
      "MailFromDomain",
      mailFromDomain,
      "Custom MAIL FROM domain (needs MX + SPF)",
    );
    out(
      "MailFromMxValue",
    ...
    out(
      ...
      "SPF TXT for MAIL FROM",
    );
    out("SesRegion", this.region, "SES region");
    out("BouncesTopicArn", bouncesTopic.topicArn, "SNS topic ARN for bounces");
    out(
      "ComplaintsTopicArn",
      complaintsTopic.topicArn,
      "SNS topic ARN for complaints",
    );
    out(
      "AlertsTopicArn",
      alertsTopic.topicArn,
      "SNS topic for CloudWatch SES alarms — subscribe your alert email here",
    );
