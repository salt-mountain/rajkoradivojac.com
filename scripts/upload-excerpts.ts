...
if (!ACCOUNT || !KEY || !SECRET) {
  console.error(
    "Set R2_ACCOUNT_ID, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY in the environment.",
  );
  process.exit(1);
}
...
for (const file of files) {
  ...
  if (!book) {
    console.warn(
      `SKIP ${file}: "${slug}" is not a known excerpt slug — rename to <slug>.pdf.`,
    );
    continue;
  }
  ...
  console.log(
    `UPLOADED excerpts/${slug}.pdf (${(body.length / 1024).toFixed(0)} KB)`,
  );
  uploaded++;
}
